...
class Element(neopixel.NeoPixel):
    ...
    def __init__(self, n = 1, colors = (COLOR_DEFAULT_CURSOR,), transparent = True, bpp = 3):
        self.n = n
...
class NeoPixel(neopixel.NeoPixel):
    ...
    def place_element(self, element, at_idx = 0):
        ...
        if element.transparent:
            for i in range(element.n):
                self[at_idx + i] = \
                    tuple(current_value + ele for current_value, ele in zip(self[at_idx + i], element[i]))
        else:
            for i in range(element.n):
    ...
    def remove_element(self, element):
        if element.transparent:
            for i in range(element.n):
                self[element._at_index + i] = \
                    tuple(current_value - ele for current_value, ele in zip(self[element._at_index + i], element[i]))
        else:
            for i in range(element.n):
    ...
    def cycle(self, element = Element(), wait_ms = DEFAULT_WAIT_ms):
        for i in range(self.n):
